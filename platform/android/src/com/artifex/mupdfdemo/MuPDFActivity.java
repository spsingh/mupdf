...
public class MuPDFActivity extends Activity implements FilePicker.FilePickerSupport
{
	...
	private boolean isAnnotatedPdf;
	public static final String KEY_TITLE = "com.artifex.mupdfdemo.mupdfactivity.KEY_TITLE";
	public static final String KEY_HEADER_COLOR = "com.artifex.mupdfdemo.mupdfactivity.KEY_HEADER_COLOR";
	public static final String KEY_ANNOTATIONS_ENABLED = "com.artifex.mupdfdemo.mupdfactivity.KEY_ANNOTATIONS_ENABLED";
	public static final String KEY_IS_ANNOTATED_PDF = "com.artifex.mupdfdemo.mupdfactivity.KEY_IS_ANNOTATED_PDF";
	public static final String KEY_SAVE_RESULTS = "com.artifex.mupdfdemo.mupdfactivity.KEY_SAVE_RESULTS";
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		this.title = intent.getStringExtra("com.artifex.mupdfdemo.mupdfactivity.KEY_TITLE");
		this.headerColor = intent.getStringExtra("com.artifex.mupdfdemo.mupdfactivity.KEY_HEADER_COLOR");
		this.annotationsEnabled = intent.getBooleanExtra("com.artifex.mupdfdemo.mupdfactivity.KEY_ANNOTATIONS_ENABLED", false);
		this.isAnnotatedPdf = intent.getBooleanExtra("com.artifex.mupdfdemo.mupdfactivity.KEY_IS_ANNOTATED_PDF", false);
	...
	public void onConfigurationChanged(Configuration newConfig) {
		...
		if (newConfig.orientation == Configuration.ORIENTATION_LANDSCAPE) {
			if (core != null) {
				...
				if (pageView != null) {
					Log.d("Orientation: ", "Updating view...");
					pageView.update();
				}
			}
	...
	public void createUI(Bundle savedInstanceState) {
		...
		if (this.title == null) {
			...
			this.mFilenameView.setText(this.title);
		}
		if ((this.headerColor != null) && (this.mTopBarMain != null)) {
			this.mTopBarMain.setBackgroundColor(Color.parseColor(this.headerColor));
		}
		...
		if ((this.annotationsEnabled) && core.fileFormat().startsWith("PDF") && core.isUnencryptedPDF() && !core.wasOpenedFromBuffer())
		{
	...
	private JSONObject saveDoc()
	{
		String newAnnotatedFileName = null;
		String newAnnotatedFilePath = null;
		String newAnnotatedFileParentFolder = null;
		String newAnnotatedFileTimestamp = null;
		String outputPath;
		if (this.isAnnotatedPdf)
		{
			outputPath = this.mPath;
		}
		else
		{
			String filesDirPath = getFilesDir().getPath();
			Uri.Builder uriBuilder = Uri.parse(filesDirPath).buildUpon();
			uriBuilder.appendPath("notes");
			
			Uri currentPathUri = Uri.parse(this.mPath);
			List<String> pathSegments = currentPathUri.getPathSegments();
			newAnnotatedFileParentFolder = (String)pathSegments.get(pathSegments.size() - 2);
			
			uriBuilder.appendPath(newAnnotatedFileParentFolder);
			Uri finalPathUri = uriBuilder.build();
			File fileDir = new File(finalPathUri.toString());
			fileDir.mkdirs();
			
			String fileName = this.mFileName.substring(0, this.mFileName.lastIndexOf('.'));
			newAnnotatedFileName = fileName;
			long now = System.currentTimeMillis() / 1000L;
			newAnnotatedFileTimestamp = String.valueOf(now);
			String finalFileName = fileName + "_" + newAnnotatedFileTimestamp + ".pdf";
			
			newAnnotatedFilePath = uriBuilder.appendPath(finalFileName).build().toString();
			
			outputPath = newAnnotatedFilePath;
		}
		if (outputPath != null)
		{
			// TODO: add support for passing in output path
			this.core.save();
			if (!this.isAnnotatedPdf)
			{
				JSONObject newAnnotationResults = new JSONObject();
				try
				{
					newAnnotationResults.put("newAnnotatedFileName", newAnnotatedFileName);
					newAnnotationResults.put("newAnnotatedFilePath", newAnnotatedFilePath);
					newAnnotationResults.put("newAnnotatedFileParentFolder", newAnnotatedFileParentFolder);
					newAnnotationResults.put("newAnnotatedFileTimestamp", newAnnotatedFileTimestamp);
		...
		if (core != null && core.hasChanges()) {
			DialogInterface.OnClickListener listener = new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					if (which == -1)
					{
						JSONObject results = MuPDFActivity.this.saveDoc();
						if (results == null) {
							results = new JSONObject();
						}
						Intent resultIntent = new Intent();
						resultIntent.putExtra("com.artifex.mupdfdemo.mupdfactivity.KEY_SAVE_RESULTS", results.toString());
						MuPDFActivity.this.setResult(-1, resultIntent);
					}
					else
					{
						Intent resultIntent = new Intent();
						resultIntent.putExtra("com.artifex.mupdfdemo.mupdfactivity.KEY_SAVE_RESULTS", "");
						MuPDFActivity.this.setResult(-1, resultIntent);
					}
